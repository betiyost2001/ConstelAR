...
const RAW_BASE = ((import.meta as any).env?.VITE_API_BASE || "/api").trim();
...
// ── Alias UI ↔ API ───────────────────────────────────────────────────────────
const UI_TO_API: Record<PollutantType, string> = {
  pm25: "pm2_5",
  o3: "ozone",
...
const API_TO_UI = Object.fromEntries(
  Object.entries(UI_TO_API).map(([k, v]) => [v, k])
);
const toApiParam = (p: PollutantType): string => UI_TO_API[p] || p;
const toUiParam = (p: string): PollutantType =>
  (API_TO_UI[p] as PollutantType) || (p as PollutantType);
...
const cacheKey = ({
  ...
  bbox: number[];
}): string => `${pollutant}:${bbox.join(",")}`;
const setCache = (k: string, v: any): void => {
  cache.set(k, { value: v, exp: Date.now() + TTL_MS });
};
const getCache = (k: string): any => {
...
// ── Helpers ──────────────────────────────────────────────────────────────────
export function bboxFromMap(map: any): number[] {
...
export async function fetchMeasurements({
  ...
  pollutant,
  signal,
}: FetchMeasurementsParams): Promise<GeoJSONFeatureCollection> {
  const [w, s, e, n] = bbox;
  const lat = (s + n) / 2;
  const lon = (w + e) / 2;
  const radiusMeters = Math.min(
    Math.hypot(
      (e - w) * 111_000 * Math.cos((lat * Math.PI) / 180),
      (n - s) * 111_000
    ) / 2,
    50_000
  );
...
export async function fetchAtPoint({
  ...
  pollutant = "pm25",
  signal,
}: FetchAtPointParams): Promise<PointMeasurement | null> {
  const url = apiUrl("openaq/normalized");
  url.searchParams.set("lat", lat.toFixed(5));
  url.searchParams.set("lon", lon.toFixed(5));
  ...
  const byPollutant = list
    .map((r: any) => ({ ...r, parameter: toUiParam(r.parameter) }))
    .filter((r: any) => r.parameter === pollutant);
  ...
  return pickedRaw
    ? { ...pickedRaw, value: toNumberSafe(pickedRaw.value) }
    : null;
}
